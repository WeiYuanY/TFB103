...
for i in range(len(jsonData)):
    ...
    #安定性
    try:
        stable = newJsonData['INJECT']['STABILITY']
    except KeyError:
        stable = "無"
...
#=======================================================================================================================
    ...
    noticesData = ''
    noticesData += drugCode + '*'
    noticesData += bagName + '*'
    noticesData += cnName + '*'
    noticesData += use + '*'
    noticesData += mainSideEffects + '*'
    noticesData += storage + '*'
    noticesData += medicationTime + '*'
    noticesData += otherInstructions + '*'
    noticesData += drugPic


    noticesDatas = re.sub('<(.|/n)+?>','',noticesData)
    noticesDatas = noticesDatas.replace('&nbsp;','').replace('&lt;','<').replace('&quot;','"').replace('&ge;','≥').replace('&sim;','∼').replace('&deg;','°C')

    DRUGCODE = noticesDatas.split('*')[0]
    EGNAME = noticesDatas.split('*')[1]
    CNNAME = noticesDatas.split('*')[2]
    CLINICALUSE = noticesDatas.split('*')[3]
    SIDEEFFECTS = noticesDatas.split('*')[4]
    STORAGEMETHOD = noticesDatas.split('*')[5]
    DRUGNOTE = noticesDatas.split('*')[6]
    OTHER = noticesDatas.split('*')[7]
    PIC = noticesDatas.split('*')[8]


    loginInfo = {'host': 'localhost','port': 3306,'user': '使用者',
                 'passwd': '密碼','db': '資料庫','charset': 'utf8mb4'}
    conn = pymysql.connect(**loginInfo)
    cursor = conn.cursor()
    cursor.execute("""INSERT INTO NOTICE(DRUG_NO,EGNAME,CNNAME,CLINICALUSE,
                    SIDEEFFECTS,STORAGEMETHOD,DRUGNOTE,OTHER,PIC)
                    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                   (DRUGCODE,EGNAME,CNNAME,CLINICALUSE,SIDEEFFECTS,
                    STORAGEMETHOD,DRUGNOTE,OTHER,PIC))
    conn.commit()
    cursor.close()
    conn.close()
